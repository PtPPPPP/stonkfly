...
- At most 24 orders with a nonzero fill per UTC day and at least 60 seconds between attempts. Rejections and zero-fill cancellations do not consume the daily fill cap. OKX's separately configured lifetime submission budget counts rejected and uncertain submissions too. Failed orders do not become new strategy choices.
...
A missing or corrupt committed checkpoint stops recovery. An older brain snapshot cannot automatically replace it while accounting and observations remain at a newer tick. Restore the verified committed artifact or investigate the state explicitly. Offline pretraining also refuses to replace the candle dataset of an existing ledger, including through `--refresh`; use a new run directory for new data. Pretraining and checkpoint deployment take the same worker locks as the trading loop. Deployment writes a separate, hash-named checkpoint before changing the ledger reference.
